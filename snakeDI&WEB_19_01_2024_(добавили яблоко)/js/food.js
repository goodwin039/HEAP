...
const _getFreeSpace = () => {//функция ищет свободное место и добавляет туда яблоко
    ...
    while(isNewCordsFood){//бесконечный цикл пока не найдём новые координаты для яблока
        x = _getRandomPosition(row);
        y = _getRandomPosition(row);

        for(let t = 0; t < tail.length; t++){//если координаты змеи совпадают с координатами ябллока, то didAte = true
